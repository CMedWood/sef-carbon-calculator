...
st.markdown(\"\"\"
    <style>
        .sef-header { background: linear-gradient(90deg, #0ea5e9 0%, #10b981 100%);
                      color: white; border-radius: 12px; padding: 14px 18px; margin-bottom: 20px; }
...
st.markdown(\"\"\"This calculator estimates **Scope 1** and **Scope 2** emissions for Australian veterinary clinics.
...
scope_df = pd.DataFrame({
    ...
    "Emissions (kg CO2e)": [scope1, scope2],
})
pie = px.pie(scope_df, names="Scope", values="Emissions (kg CO2e)", title="Emissions by Scope", hole=0.35)
st.plotly_chart(pie, use_container_width=True)

by_cat = res.iloc[:-1].copy()  # exclude Total row
bar = px.bar(by_cat, x="Metric", y="Value (kgCO2e)", title="Emissions by Category", text_auto=".2s")
